...
public class QuoteSubmit {
	...
	public static final QuoteSubmit instance(String username, String password, String productCode, VehicleOrderParam param) {
		QuoteSubmit submit = new QuoteSubmit();
		submit.setProductCode(productCode);
		if (StringUtil.hasText(param.getCommercialStart())) {
			submit.setCommercePolicyBeginDate(DateUtil.convert(param.getCommercialStart(), DateUtil.YYYY_MM_DD_HH_MM_SS, DateUtil.YYYY_MM_DDTHH_MM_SS, DateUtil.TIMEZONE_GMT_8));
			submit.setCommercePolicyEndDate(DateUtil.dateOyearTail(param.getCommercialStart(), DateUtil.YYYY_MM_DD_HH_MM_SS, DateUtil.YYYY_MM_DDTHH_MM_SS, DateUtil.TIMEZONE_GMT_8));
		}
		if (StringUtil.hasText(param.getCompulsoryStart())) {
			submit.setCompulsoryPolicyBeginDate(DateUtil.convert(param.getCompulsoryStart(), DateUtil.YYYY_MM_DD_HH_MM_SS, DateUtil.YYYY_MM_DDTHH_MM_SS, DateUtil.TIMEZONE_GMT_8));
			submit.setCompulsoryPolicyEndDate(DateUtil.dateOyearTail(param.getCompulsoryStart(), DateUtil.YYYY_MM_DD_HH_MM_SS, DateUtil.YYYY_MM_DDTHH_MM_SS, DateUtil.TIMEZONE_GMT_8));
		}
		
		submit.setLogUser(new LogUser(username, password));
		
		InsuredInfo insuredInfo = new InsuredInfo();
		Unit unit = param.getInsured();
		...
		vehicleInfo.setLicenseNo(param.getLicense());
		vehicleInfo.setVin(param.getVin());
		vehicleInfo.setEngineNo(param.getEngine());
		vehicleInfo.setID(param.getVehicleId());
		vehicleInfo.setEnrollDate(DateUtil.convert(param.getEnrollDate(), DateUtil.YYYY_MM_DD_HH_MM_SS, DateUtil.YYYY_MM_DDTHH_MM_SS, DateUtil.TIMEZONE_GMT_8));
		vehicleInfo.setTransferFlag(param.isTransfer() ? 1 : 0);
		vehicleInfo.setTransferFlagTime(!StringUtil.hasText(param.getIssueDate()) ? null : DateUtil.convert(param.getIssueDate(), DateUtil.YYYY_MM_DD_HH_MM_SS, DateUtil.YYYY_MM_DDTHH_MM_SS, DateUtil.TIMEZONE_GMT_8));
		vehicleInfo.setUseNature(_useNature(param.getUsedType()));
...
//		vehicleInfo.setLoadWeight(tips.getLoad());
		...
		submit.setVehicleInsurance(LeBaoBaInsurance.insuranceMapping(param, submit.getCommercePolicyBeginDate(), DateUtil.convert(param.getEnrollDate(), DateUtil.YYYY_MM_DD_HH_MM_SS, DateUtil.YYYY_MM_DDTHH_MM_SS, DateUtil.TIMEZONE_GMT_8)));
		return submit;
	}
